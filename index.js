...
export function isRepo(repoURL) {
    fetch(repoURL + "/CydiaIcon.png").then(function(){
        return true;
    }).catch(function(){
        return false;
    })
...
export async function downloadPackageFromRepo(repoURL, packageName, downloadLocation) {
    const repo = await this.parsePackages(repoURL);
...
export async function downloadAllPackagesFromRepo(repoURL, downloadLocation) {
    ...
    const repoDownload = JSON.parse(await this.parsePackages(repoURL));
    let errors = []
    for (var pkg in repoDownload.packages) {
        try {
            const response = await fetch(repoDownload.packages[pkg].Filename, {
...
export async function parseRepo(repoURL) {
    ...
    // Release data
    const release = await fetch(repoURL+"/Release", {
        headers: headers
    }).catch(function(){
        return false;
    });
    let parsedRelease = await debianCtrl.parse(await release.text());
    let data = {
        "name": parsedRelease.Label,
        "version": parsedRelease.Version,
        "codename": parsedRelease.codename,
        "architecture": parsedRelease.Architectures,
        "iconURL": repoURL + "/CydiaIcon.png",
        "url": repoURL,
        "packages": []
};
...
export async function parsePackages(repoURL) {
    let skip = false;
    const tempDir = fs.mkdtempSync("repoDownloader")
    try {
        const response = await fetch(repoURL+"/Packages", {
